...
class Heist(commands.Cog):
  ...
  async def bail(self,ctx, member:discord.Member=None):
    """ Bail somebody out """
    if member == None:
      em = discord.Embed(description=f'Who do you want to bail?', color=botcolour)
      return await ctx.send(embed=em)
    elif member == ctx.author:
      em = discord.Embed(description=f'You cannot bail yourself', color=botcolour)
      return await ctx.send(embed=em)
    user = ctx.author
    await open_server(user)
    await open_account(user)
    ...
    if status["status"] != "Jailed":
      em = discord.Embed(description=f'What are you doing? {member.display_name} is {status["status"]}', color=botcolour)
      return await ctx.send(embed=em)
    if users["wallet"] < 20000:
      em = discord.Embed(description = f"You need 20,000 {currency} for bailing", colour = discord.Color.red())
      return await ctx.send(embed = em)
    mainbank.update_one({"_id":user.id}, {"$inc":{"wallet":-20000}})
    centralbank.update_one({"_id":member.id}, {"$set":{"status":"Alive"}})
    em = discord.Embed(description=f'{user.mention} {member.display_name} has been bailed out with 20000 {currency}', color=user.color)
    return await ctx.send(embed=em)
  ...
  async def bonk(self,ctx, member:discord.Member=None):
    ...
    await open_account(member)
    await open_heist_account(member)
    centralbank.update_many({"_id":member.id}, {"$set": {"status": "Dead", "timer": time.time()}})
    em = discord.Embed(description=f'{user.mention} `{member.display_name}` is now `Dead`', color=user.color)
    return await ctx.send(embed=em)
  ...
  async def jail(self,ctx, member:discord.Member=None):
    ...
    await open_account(member)
    await open_heist_account(member)
    centralbank.update_many({"_id":member.id}, {"$set": {"status": "Jailed", "timer": time.time()}})
    em = discord.Embed(description=f'{user.mention} `{member.display_name}` is now `Jailed`', color=user.color)
    return await ctx.send(embed=em)
  
  @commands.command(aliases=['revive'])
  async def resurrect(self,ctx, member:discord.Member=None):
    """ Resurrect somebody earlier """
    if member == None:
      em = discord.Embed(description=f'Who do you want to resurrect?', color=botcolour)
      return await ctx.send(embed=em)
    elif member == ctx.author:
      em = discord.Embed(description=f'You cannot resurrect yourself', color=botcolour)
      return await ctx.send(embed=em)
    user = ctx.author
    await open_server(user)
    await open_account(user)
    ...
    if status["status"] != "Dead":
      em = discord.Embed(description=f'What are you doing? {member.display_name} is {status["status"]}', color=botcolour)
      return await ctx.send(embed=em)
    if users["wallet"] < 500000:
      em = discord.Embed(description = f"You need 500,000 {currency} for resurrection", colour = discord.Color.red())
      return await ctx.send(embed = em)
    mainbank.update_one({"_id":user.id}, {"$inc":{"wallet":-500000}})
    centralbank.update_one({"_id":member.id}, {"$set":{"status":"Alive"}})
    em = discord.Embed(description=f'{user.mention} {member.display_name} has been revived with 500000 {currency}', color=user.color)
    return await ctx.send(embed=em)
...
async def heist_process(target, crewlist):
  ...
  secondroll = random.randint(1,75)
  death = []
  survivor = crewlist
  ...
  if roll <= probability: # winning the heist
    ...
    if secondroll <= probability and len(survivor) == 0:
      lucky1 = random.choice(death)
      death.remove(lucky1)
      survivor.append(lucky1)
      rob_amount/=2
