...
class SDSGC(commands.Cog):
  ...
  async def pull(self,ctx,arg1=None):
    ...
    if bal[0] < 30:
      ...
      em.set_footer(text= "try !timely")
      return await ctx.send(embed = em)
    banner_list = sorted(["T1","VAL","AM","DZ","MERLIN","FZEL", "REZERO", "ST", "EXARTH", "FBAN", "FGOW", "FKING","HAWK","SEASON", "RAG1", "RAG2", "RAG3", "SROXY","SMLH", "LUDO", "CUSACK", "EMO", "HMATRONABanner", "HDIANE", "XLILLIA", "KOF"])
    d={}
    out = []
    if arg1 == "list":
      n = 10
    ...
    dirs = []
    ban6p = ["DZ"]
    ban4p = ["T1","AM","MERLIN","FZEL","FBAN","LUDO", "CUSACK", "FGOW", "FKING"]
    ban3p = ["VAL","REZERO","ST","EXARTH","HAWK","SEASON", "RAG1", "RAG2", "RAG3", "SROXY", "SMLH", "EMO", "HMATRONABanner", "HDIANE", "XLILLIA", "KOF"]
